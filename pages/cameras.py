    ...
    with PageLayout('/cameras','Red de cámaras','Vistas de prueba de la red distribuida. Selecciona una cámara para consultar su información.'):
        ...
        def open_camera(cid):
            ...
            with detail:
                ...
                CameraFeed(camera)
                own=camera.id==live.camera_id
                with ui.column().classes('p-5 w-full gap-2'):
                    StatusBadge(camera.status)
                    audio='Micrófono del equipo · detección por voz' if camera.id==config.DEFAULT_CAMERA_ID else 'Disponible · simulación' if camera.audio else 'No disponible'
                    ...
                    if own:
                        InfoPair('Video','Cámara del equipo · '+('en vivo' if live.running else 'detenida'))
        ...
        def grid():
            ...
            cameras.sort(key=lambda c:c.id!=live.camera_id)  # the webcam of this computer first
            if not cameras:
                EmptyState()
                return
            ...
            ui.label(f'{min(start+1,len(cameras))}–{min(start+count*count,len(cameras))} de {len(cameras)} cámaras · {live.camera_id} es la cámara de este equipo; las demás son capturas sintéticas').classes('text-xs muted mt-3')
