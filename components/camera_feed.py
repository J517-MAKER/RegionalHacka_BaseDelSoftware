...
def LiveStage(camera):
    ...
    with ui.element('div').classes('camera-overlay'):
        with ui.element('div').classes('camera-top'):
            ui.label(camera.id)
            badge=ui.label('● EN VIVO').style('background:#c62828;padding:2px 6px')
        with ui.element('div').classes('camera-bottom'):
            ui.label('CÁMARA DEL EQUIPO')
            ui.icon('volume_up' if camera.audio else 'volume_off',size='13px')

    def update():
        running=live.running and live.camera_id==camera.id
...
def CameraFeed(camera,on_select=None):
    own=camera.id==live.camera_id  # the webcam of this computer stands in for this camera
